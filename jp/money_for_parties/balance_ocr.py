...
def get_question_constraint(index):
    ...
    if index == 1:
        ...
        # 寄付の内訳
        questions.append({"name": "寄付の内訳", "type": "csv", "question": "寄付者の氏名（又は名称）,金額,年月日,住所（又は所在地）,職業（又は代表者の氏名）,備考をCSV形式で出力してください。情報がない行,計や合計を含めないでください。金額は句読点なしで数字のみで出力してください。年月日は/で分けてください。説明は含めずcsvのみを出力してください。出力は```\n寄付者の氏名（又は名称）,金額,年月日,住所（又は所在地）,職業（又は代表者の氏名）,備考で始めてください", "extra_body": {
            "guided_regex": "```\n寄付者の氏名（又は名称）,金額,年月日,住所（又は所在地）,職業（又は代表者の氏名）,備考\n([^,]*,[0-9]*,[^,]*,[^,]*,[^,]*,[^,]*\n)*```[\s\S]*"
        }})
        constraints.append({"type": "csv_add", "lhs": "寄付の内訳/金額", "rhs": "この頁の小計"})
        questions.append({"name": "この頁の小計", "type": int, "question": "この頁の小計は？句読点なしの数字のみで出力してください。もし空白なら0と出力してください。", "extra_body": {"guided_regex": "[0-9]+"}})
